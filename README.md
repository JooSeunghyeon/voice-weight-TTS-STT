...
`TTS`와 XTTS-v2는 최신 `transformers` 일부 버전과 호환되지 않으므로 `transformers==4.40.2`로 고정합니다.

## Local Run

Python 3.11 환경과 CUDA GPU를 권장합니다.
...
3. Kaggle 기본 Python이 `TTS` 패키지와 맞지 않을 수 있으므로, 노트북에서 Miniforge 기반 Python 3.11 환경을 만든 뒤 실행합니다.
